...
def load_data():
    try:
        ...
        # Filter accounts for Odoo consistency
    ...
    for partner in selected_partners:
        ...
        <div class="page-container">
            ...
            <div style="font-size: 14px; margin-bottom: 20px;">
                ...
                <strong>Generated on:</strong> {pd.Timestamp.now().strftime('%Y-%m-%d')}
            </div>
            <table>
                <thead>
                    <tr>
...
if not df.empty:
    ...
    with tab1:
        ...
        with col1:
            date_range = st.date_input("Select Period:", [df['date'].min(), df['date'].max()], key="dr_main")
        with col2:
            partner_options = sorted(df['partner_id'].unique().tolist())
            selected_partners = st.multiselect("Select Partners:", options=partner_options)
        ...
        if selected_partners:
            ...
            for partner in selected_partners:
                ...
                if not p_data.empty:
                    ...
                    with st.expander(f"Preview Ledger: {partner}", expanded=True):
                        ...
                        m1.metric("Total Debit", f"{p_data['debit'].sum():,.2f}")
                        m2.metric("Total Credit", f"{p_data['credit'].sum():,.2f}")
            ...
            if st.button("🚀 Download Statements (PDF)"):
                with st.spinner("Generating PDF..."):
                    try:
                        pdf_bytes = generate_pdf_multi_page(filtered_df, selected_partners)
                        st.download_button("📥 Save PDF Report", pdf_bytes, "Suhul_Albeeah_Ledgers.pdf")
                    except Exception as e:
